 ...
 * and then removed using page addLinesByPassAuto / removeLinesByPass methods.
 *
 * @example
 *
 *     // With MyTablePage extending TablePage
...
TableAssert.prototype.checkItemsPerPage = function() {
  ...
  return this.page.search(search).then(function() {
    ...
    // Add lines 6 lines
    self.page.addLinesByPassAuto(name, i + 6).then(function(addedLines) {
    ...
    self.page.addLinesByPassAuto(name, 5, i + 1).then(function(addedLines) {
      self.page.search(search);
      lines = lines.concat(addedLines);
    });
    i += 5;
...
TableAssert.prototype.checkLinesSelection = function(columnTitle) {
  ...
  return browser.waitForAngular().then(function() {
    ...
    // Create lines
    self.page.addLinesByPassAuto(name, 2).then(function(addedLines) {
      lines = addedLines;
    }).then(function() {
