...
TablePage.prototype.getLineFieldValues = function(lineFinder) {
  ...
  function getFieldText(field, fieldName) {
...
TablePage.prototype.getGlobalActions = function() {
  var deferred = protractor.promise.defer();
  var actions = [];
  var actionElements = this.actionsElement.all(by.css('a'));

  this.selectAllLines();
  browserExt.click(this.actionsButtonElement);
  browser.wait(this.EC.visibilityOf(this.actionsElement), 1000, 'Missing the list of actions').then(function() {
...
TablePage.prototype.removeLine = function(lineFinder) {
  return this.performAction(lineFinder, this.translations.UI.REMOVE, true);
};
 ...
 * @param {Boolean} [refresh=true] Request for a refresh after adding lines
 * @return {Promise} Promise resolving when lines are added and browser page has been reloaded
 */
TablePage.prototype.addLinesByPass = function(lines, refresh) {
  var self = this;
  ...
  return browser.waitForAngular().then(function() {
    ...
    function createAddFunction(line) {
      ...
      parallel.push(function(callback) {
        ...
        // Add line
        self.model.add(line, function(error, addedCount, addedLine) {
 ...
 */
TablePage.prototype.addLinesByPassAuto = function(name, total, offset, refresh) {
  var lines = [];
  offset = offset || 0;
 ...
 */
TablePage.prototype.removeLinesByPass = function(lines, refresh) {
  var self = this;
  ...
  return browser.waitForAngular().then(function() {
    ...
    self.model.remove(lineIds, function(error) {
      if (error) {
        deferred.reject(error);
      } else {
        deferred.fulfill();
