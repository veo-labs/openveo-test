...
/**
 * A back end page to help writing end to end tests on OpenVeo back end pages.
 *
 * Do not use this directly but extend it.
 *
 * @example
 *
 *     var BackEndPage = require('@openveo/test').e2e.BackEndPage;
 *
...
function BackEndPage() {
   ...
   */
  this.alertElements = element.all(by.css('.alert'));
   ...
   * @type ElementFinder
   */
  this.popoverElement = element(by.css('.popover'));
   ...
   * @type ElementFinder
   */
  this.toggleLeftMenuLinkElement = element(by.css('.navbar-header button'));
   ...
   * @type ElementFinder
   */
  this.leftMenuElement = element(by.id('sidebar-wrapper'));
   ...
   * @type ElementFinder
   */
  this.profileLinkElement = element(by.css('.nav a[href="profile"]'));
   ...
   * @type ElementFinder
   */
  this.languageLinkElement = element(by.css('.nav .language > a'));
   ...
   * @type ElementFinder
   */
  this.logoutLinkElement = element(by.css('.nav a[href="logout"]'));
   ...
   * @type ElementArrayFinder
   */
  this.level1MenuLinkElements = element.all(by.css('#sidebar-wrapper > ul > li'));
...
function clickMenuRecursive(elements, itemName) {
  ...
  elements.each(function(menuItem, index) {
    ...
    // Get menu item text
    menuLink.getText().then(function(text) {
      if (text === itemName) {
        ...
        return subMenu.isPresent().then(function(isPresent) {
          ...
          return clickMenuRecursive.call(self, menuItem.all(by.css('.sub-menu > li')), itemName);
...
/**
 * Gets a level 1 menu items.
 *
 * All elements corresponding to the item name will be returned.
 *
 * @example
 *
 *     // With MyBackEndPage extending BackEndPage
 ...
 * @param {String} itemName The translated name of the menu item
 * @return {Promise} Promise resolving with the list of elements corresponding to the given name
 */
BackEndPage.prototype.getLevel1MenuItems = function(itemName) {
  var deferred = protractor.promise.defer();

  this.level1MenuLinkElements.filter(function(menuLink, index) {
    return menuLink.element(by.xpath('./a/span')).getText().then(function(text) {
...
BackEndPage.prototype.closeAlerts = function() {
  ...
  this.alertElements.each(function(alertElement, index) {
